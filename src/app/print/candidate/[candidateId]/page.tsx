...
export default async function PrintCandidatePage({ params, searchParams }: Props) {
  ...
  const [answersRes, analysisRes, cachedRes, allCandidatesRes, overridesRes] = await Promise.all([
    ...
    useManual
      ? db.from("manual_score_answer_overrides").select("candidate_answer_id, manual_score_achieved, manual_max_score").eq("manual_score_id", manualScore!.id)
      : Promise.resolve({ data: null } as any),
  ])
  ...
  const groupLogos: string[] = useManual ? (exam?.courses?.groups?.manual_report_logos ?? []) : []
    ...
    <>
      ...
      <div id="report-root" style={{ width: 794, margin: "0 auto", display: "flex", flexDirection: "column", background: "white" }}>
        ...
        <Page dark first>
          ...
          <div className="relative px-12 py-6 border-t border-white/10 flex items-center justify-between shrink-0">
            ...
            {useManual && (
              <p className="absolute left-1/2 -translate-x-1/2 text-white/30 text-[10px]">{today}</p>
            )}
