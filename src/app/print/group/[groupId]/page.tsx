  ...
  const courseDataArr = await Promise.all(
    courses.map(async (course: any) => {
      ...
      const examDataArr = await Promise.all(exams.map(async (exam: any) => {
        const candidatesRes = await db.from("candidates").select("id, full_name, company, started_at, submitted_at, total_score, passed").eq("exam_id", exam.id).not("submitted_at", "is", null)
        ...
        // from their own draw instead. Mirrors the API route's
        // fetchGroupData exactly, since this page fetches independently
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                        {courseLeaderboard.slice(0, 5).map((c: any, idx: number) => {
                          const col = isManual ? letterGrade(c.avgScore) : scoreColor(c.avgScore)
                          return (
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {allSubmissions.map((c: any, idx: number) => {
                      const col = isManual ? letterGrade(c.total_score ?? 0) : scoreColor(c.total_score ?? 0)
                      return (
