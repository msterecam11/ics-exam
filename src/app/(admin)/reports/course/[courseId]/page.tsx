...
import {
  ...
  Trophy, Target, TrendingUp, Lightbulb, Users, Medal, Clock,
} from "lucide-react"
...
// ─── Layout primitives (mirror print page) ──────────────────────────────────
...
// ─── Expert placeholder (view-only, hidden in PDF) ───────────────────────────
...
export default function CourseReportViewPage() {
  ...
  useEffect(() => {
    setLoading(true)
    fetch(`/api/reports/course/${courseId}${mode === "manual" ? "?mode=manual" : ""}`)
      .then(r => r.json())
  ...
  async function downloadPDF() {
    ...
    try {
      const res = await fetch(
        `/api/reports/course/${courseId}/pdf?entity=${encodeURIComponent(entityTerm)}&content=${encodeURIComponent(contentTerm)}${mode === "manual" ? "&mode=manual" : ""}`
      )
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-slate-100">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-slate-500">{t("Loading course report…")}</p>
        </div>
      </div>
    )
  }
    ...
    <>
      ...
      <div className="bg-slate-300 min-h-screen py-8 flex justify-center">
        <div id="report-root" className="flex flex-col gap-5" style={{ width: 794 }}>
          ...
          <Page dark first>
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-12 text-center gap-8">
              ...
              <div className="flex items-center gap-10">
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold text-white">{mode === "manual" ? letterGrade(overallAvg).letter : `${overallAvg.toFixed(1)}%`}</p>
            ...
            <div className="px-12 py-7 space-y-6">
              ...
              {narrative?.course_summary ? (
                <div className="bg-[#1B4F8A]/5 border border-[#1B4F8A]/10 rounded-xl p-5 avoid-break">
                  ...
                  <p className="text-sm text-slate-700 leading-relaxed">{t(narrative.course_summary)}</p>
                </div>
              ) : (
                      ...
                      {exams.map(({ exam, candidateCount, avgScore, passCount, passRate }: any, idx: number) => {
                        ...
                        return (
                          <tr key={exam.id} className={`border-b border-slate-50 last:border-0 ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                            ...
                            <td className="px-4 py-2.5 text-xs text-slate-600">{candidateCount}</td>
                            <td className="px-4 py-2.5">
                              <span className="text-xs font-bold" style={{ color: col.text }}>{mode === "manual" ? letterGrade(avgScore).letter : `${avgScore.toFixed(1)}%`}</span>
                            </td>
                            ...
                            <td className="px-4 py-2.5">
                              <span className="text-xs font-semibold text-red-500">{candidateCount - passCount}</span>
                            </td>
                          </tr>
                        )
                      })}
                          ...
                          <tr key={`${c.id}-${idx}`} className={`border-b border-slate-50 last:border-0 ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                            ...
                            <td className="px-4 py-2">
                              <span className="text-xs font-bold" style={{ color: col.text }}>
                                {mode === "manual" ? letterGrade(c.total_score ?? 0).letter : `${(c.total_score ?? 0).toFixed(1)}%`}
                              </span>
                            </td>
                            <td className="px-4 py-2">
                              {c.passed ? (
          ...
          {/* ══ PAGES 4+ — PER-EXAM ══ */}
          {exams.map(({ exam, candidateCount, avgScore, passCount, passRate, sectionAvgs }: any, examIdx: number) => {
            const col = scoreColor(avgScore)
              ...
              <Page key={exam.id}>
                ...
                <div className="px-12 py-7 space-y-5">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <span className="text-[10px] font-bold uppercase tracking-widest text-[#1B4F8A] bg-blue-50 px-2 py-0.5 rounded-full">
                          Exam {examIdx + 1} of {exams.length}
                        </span>
                  ...
                  <div className="flex gap-2 flex-wrap avoid-break">
                    {[
                      { label: "Candidates", val: candidateCount,                  color: "bg-slate-100 text-slate-700" },
                      { label: "Passed",     val: passCount,                       color: "bg-emerald-50 text-emerald-700" },
                      { label: "Failed",     val: candidateCount - passCount,      color: "bg-red-50 text-red-600" },
                      { label: "Pass Rate",  val: `${passRate.toFixed(0)}%`,       color: "bg-blue-50 text-blue-700" },
                      { label: "Average",    val: mode === "manual" ? letterGrade(avgScore).letter : `${avgScore.toFixed(1)}%`, color: "bg-purple-50 text-purple-700" },
                    ].map(({ label, val, color }) => (
                  ...
                  {sectionAvgs.length === 0 && (
                    <div className="bg-slate-50 border border-slate-100 rounded-xl px-5 py-4 avoid-break">
                      <p className="text-xs text-slate-400 text-center">
                        No section analysis available — run "Analyze Exam" to generate section breakdowns.
                      </p>
                    </div>
                  )}
          ...
          {hasAI && (
            <Page>
              ...
              <PageFooter page={3 + exams.length + 1} total={totalPages} />
            </Page>
          )}
              ...
              <div className="px-12 py-7 space-y-6">
                ...
                <div className="border-t border-slate-100 pt-6 flex flex-col items-center gap-2 text-center avoid-break">
                  ...
                  <p className="text-[10px] text-slate-300 max-w-md">
                    {t("This report is expert-generated based on course performance data and is for internal use only.")}
                  </p>
                </div>
              </div>
