...
export async function GET(
  ...
  const printUrl = `http://localhost:${port}/print/lms/attendance/${sessionId}?pdf_secret=${secret}`
  ...
  try {
    ...
    const filename = `${sessionTitle} - Attendance ${sessionDate}.pdf`
      .replace(/[^\x00-\x7F]/g, "-").replace(/[/\\?%*:|"<>]/g, "-").replace(/-{2,}/g, "-").trim()
    const filenameEncoded = encodeURIComponent(`${sessionTitle} — Attendance ${sessionDate}.pdf`)
