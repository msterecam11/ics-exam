...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { text, score, ai_scoring_guide, image_url, section_id, choices, matching_pairs, ordering_items } = body
  ...
  await db.from("questions").update({ text, score, ai_scoring_guide, image_url: image_url ?? null, section_id: section_id ?? null }).eq("id", id)
