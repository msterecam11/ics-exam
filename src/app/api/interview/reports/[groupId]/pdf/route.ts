...
/**
 * GET /api/interview/reports/[groupId]/pdf
 * Renders the server-side print page as a per-page merged PDF via Puppeteer + pdf-lib.
 */
export async function GET(req: Request, { params }: Params) {
  ...
  const printUrl = `http://localhost:${port}/print/interview/group/${groupId}?pdf_secret=${secret}`
  ...
  try {
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      // Pass width/height directly to page.pdf() — this bypasses @page CSS entirely
      // and is the most reliable way to set exact page dimensions in Puppeteer
    ...
    const displayName     = "Interview Report - Group Report"
