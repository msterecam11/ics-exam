...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { type, text, score, order_index, ai_scoring_guide, image_url, section_id, choices, matching_pairs, ordering_items } = body

  // Insert question
  const { data: question, error: qErr } = await db
    .from("questions")
    .insert({ exam_id, type, text, score, order_index: order_index ?? 0, ai_scoring_guide: ai_scoring_guide ?? null, image_url: image_url ?? null, section_id: section_id ?? null })
    .select()
    .single()
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updates = questions.map((q: { id: string; order_index: number }) =>
    db.from("questions").update({ order_index: q.order_index }).eq("id", q.id).eq("exam_id", exam_id)
  )
