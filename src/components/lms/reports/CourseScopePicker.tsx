...
export default function CourseScopePicker({ courseId, options, programId, trackId, allRuns, month = null, monthLabel = null }: {
  courseId: string; options: Option[]; programId: string | null; trackId: string | null; allRuns: boolean
  month?: string | null; monthLabel?: string | null
}) {
  const router = useRouter()
  const go = (q: string) => router.push(`/lms-admin/reports/${courseId}/group${q ? `?${q}` : ""}`)
  const value = programId ? `p:${programId}` : month ? `m:${month}` : allRuns ? "all" : ""
  const tracks = options.find(o => o.id === programId)?.tracks ?? []

  return (
    <div className="flex items-center gap-2 flex-wrap">
      <select aria-label="Report scope" value={value}
        onChange={e => { const v = e.target.value; go(v === "all" ? "scope=all" : v.startsWith("p:") ? `program=${v.slice(2)}` : v.startsWith("m:") ? `month=${v.slice(2)}` : "") }}
        ...
        <option value="all">All groups combined (course analytics)</option>
        {month && <option value={`m:${month}`}>Individual learners · {monthLabel ?? month}</option>}
        {options.length > 0 && (
