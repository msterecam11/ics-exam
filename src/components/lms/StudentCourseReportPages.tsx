...
import {
  ...
  Medal, Star, Clock, ClipboardList, CalendarCheck, MessageSquareText,
  BrainCircuit,
} from "lucide-react"
import type { CourseReport } from "@/lib/lms-course-report"
import ScoreBar from "@/components/reports/ScoreBar"

// Read-only, fully server-renderable report pages — this is THE single
// source of truth for the individual student/course report's layout,
// shared by the interactive admin view (window.print / live preview), the
// PDF download route (Puppeteer, sliced per [data-report-page] element),
// and the Viewer Portal's read-only report page. Previously the admin view
// and the PDF/viewer pages were two independently-maintained components
// that had drifted apart — an admin downloading a PDF (or a viewer opening
// their report) got a much older, compressed 3-page layout instead of the
// richer one-section-per-page report the admin view actually shows.
// Editing report content? Edit only here — every consumer updates together.
            ...
            <div className="avoid-break">
              ...
              {assessment ? (
                ...
                <div className="no-print flex items-center gap-2 text-sm text-slate-400 bg-slate-50 rounded-xl p-4">
                  <Sparkles className="h-4 w-4 shrink-0 text-purple-400" /> No expert assessment generated for this student yet.
                </div>
              )}
            </div>
          ...
          <Page>
            ...
            <div className="px-12 py-7 space-y-6">
              ...
              {examSections.length > 0 ? (
                ...
                <div className="no-print flex items-center gap-2 text-sm text-slate-400 bg-slate-50 rounded-xl p-4">
                  <Sparkles className="h-4 w-4 shrink-0 text-purple-400" /> This exam has no section breakdown mapped to course modules.
                </div>
              )}
            </div>
          ...
          <Page>
            ...
            <div className="px-12 py-7 space-y-5">
              ...
              {security.analysis ? (
                ...
                <div className="no-print flex items-center gap-2 text-sm text-slate-400 bg-slate-50 rounded-xl p-4">
                  <Sparkles className="h-4 w-4 shrink-0 text-purple-400" /> No AI integrity assessment generated for this student yet.
                </div>
              )}
            </div>
