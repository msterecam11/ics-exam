...
const TYPE_COLORS: Record<ItemType, { color: string; bg: string }> = {
  ...
  activity:    { color: "text-purple-400", bg: "bg-purple-900/30" },
}
...
export default function PackagePlayer({
  ...
  useEffect(() => {
    ...
    const flush = () => {
      ...
      try {
        const payload = JSON.stringify({ module_id: moduleId, course_id: courseId, time_spent: secs })
  ...
  }, [previewMode, packageId, moduleId, courseId])
  ...
  function goNext() {
    ...
    // Assessed items (quiz/video/activity) must complete themselves before advancing
  ...
  const ASSESSED_TYPES: ItemType[] = ["video", "youtube", "audio", "quiz", "exam", "activity"]
        ...
        <main className="flex-1 flex flex-col min-w-0 min-h-0 relative overflow-hidden">
          ...
          {currentItem ? (() => {
            ...
            if (currentItem.type === "activity") {
              const activityData = {
                id: currentItem.id,
                type: (cfg.activity_type ?? "mcq") as any,
                title: currentItem.title,
                placement_slide: 0,
                difficulty: (cfg.difficulty ?? "medium") as any,
                ai_generated: cfg.ai_generated ?? false,
                content: cfg.content ?? {},
              }
              return (
                <div className="flex-1 overflow-y-auto bg-slate-50 flex items-start justify-center p-6">
