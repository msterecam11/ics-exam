...
export interface CandidateReportCanvasProps {
  group: {
    id: string
    name: string
    status: string
...
// ─── Inline SVG Radar Chart (SSR-safe — no recharts / dynamic import) ─────────

function PillarRadarSvg({ pillars, size = 260 }: { pillars: Array<{ name: string; score: number }>; size?: number }) {
  const n = pillars.length
  if (n < 3) return null

  const cx    = size / 2
  const cy    = size / 2
  const maxR  = size * 0.33   // radius for score = 5
  const labR  = maxR + 26     // radius for axis labels
  ...
  // Polygon path string for a given per-axis value array
  const polyPath = (vals: number[]) =>
    vals.map((v, i) => { const p = pt(i, v); return `${i === 0 ? "M" : "L"}${p.x},${p.y}` }).join(" ") + "Z"

  const candidateVals = pillars.map(p => p.score)
  ...
  return (
    ...
    >
      {/* Grid rings */}
      {rings.map(ring => (
        <path key={ring} d={polyPath(pillars.map(() => ring))} fill="none" stroke="#e2e8f0" strokeWidth={1} />
      ))}
      ...
      {/* Candidate polygon */}
      <path d={polyPath(candidateVals)} fill="#1B4F8A" fillOpacity={0.22} stroke="#1B4F8A" strokeWidth={2.5} />

      {/* Candidate dots */}
      {candidateVals.map((v, i) => {
        const p = pt(i, v)
        return <circle key={i} cx={p.x} cy={p.y} r={3.5} fill="#1B4F8A" />
      })}

      {/* Axis labels */}
      {pillars.map((p, i) => {
        const a     = angle(i)
        const lx    = cx + labR * Math.cos(a)
        const ly    = cy + labR * Math.sin(a)
        ...
        return (
          <text key={i} textAnchor="middle" fill="#64748b" fontSize={10} fontWeight={500}>
            {lines.map((line, j) => (
              <tspan key={j} x={lx} y={ly + (j - (lines.length - 1) / 2) * lineH}>{line}</tspan>
            ))}
          </text>
        )
      })}
...
function DevelopmentRoadmap({ raw }: { raw?: string }) {
  if (!raw) return null
  ...
  try {
    ...
    return <ExpertPanel text={raw} label="Expert Development Plan" color="amber" />
  }
  ...
  function itemColor(tl?: string) {
    ...
    if (months <= 3)  return { bg: "bg-emerald-50", border: "border-emerald-200", badge: "bg-emerald-100 text-emerald-700", circle: "bg-emerald-500" }
    if (months <= 6)  return { bg: "bg-amber-50",   border: "border-amber-200",   badge: "bg-amber-100 text-amber-700",     circle: "bg-amber-500"   }
    return              { bg: "bg-red-50",     border: "border-red-200",     badge: "bg-red-100 text-red-700",         circle: "bg-red-500"     }
  }
          ...
          {items.map((item, idx) => {
            ...
            return (
              <div key={idx} className="flex gap-4">
                ...
                <div className={cn("flex-1 rounded-xl border p-3 mb-3", c.bg, c.border)}>
                  <p className="text-[12px] font-bold text-slate-800 leading-snug">{label}</p>
                  {action && <p className="text-[10px] text-slate-600 leading-relaxed mt-1">{action}</p>}
...
export default function CandidateReportCanvas({
  ...
  const totalPages  = 5 + pillarPages + (hasDevPage ? 1 : 0) + 1
    ...
    <div id="report-root" className="flex flex-col" style={{ width: 794 }}>
      ...
      <Page>
        ...
        <div className="px-12 py-6 space-y-5">
          ...
          <ExpertPanel text={aiCache["executive_summary"]} label="Expert Executive Summary" color="blue" />
        </div>
        ...
        <div className="px-12 py-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6 avoid-break">
            <div>
              ...
              <PillarRadarSvg pillars={radarData} size={260} />
            </div>
            <div>
              ...
              <div key={pr.pillar.id} className="avoid-break space-y-2">
                ...
                {noScoresSubmitted && (
                  <div className="flex items-center gap-2 px-4 py-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-700">
                    ...
                    <p className="text-[10px] font-semibold">No scores submitted yet for this pillar.</p>
                  </div>
                )}
                  ...
                  <table className="w-full text-xs">
                        ...
                        {pillarAssessors.map(a => {
                          ...
                          return (
                            <th key={a.id} className="text-center text-[9px] font-bold uppercase tracking-wider text-slate-500 py-2.5 px-3 whitespace-nowrap">
                              {a.name.split(" ")[0]}
                              {wPct !== 100 && <span className="font-normal text-slate-400 ml-0.5">({wPct}%)</span>}
                            </th>
                          )
                        })}
                    ...
                    <tbody>
                        ...
                        return (
                          <tr key={cr.competency.id}
                            ...
                            {pillarAssessors.map(a => {
                              ...
                              const ac = s !== undefined ? scoreColor(s) : null
                              return (
                      ...
                      <tr className="bg-[#1B4F8A]/5 border-t-2 border-[#1B4F8A]/10">
                        <td className="py-2.5 px-4 text-[10px] font-extrabold text-[#1B4F8A] uppercase tracking-wider">Pillar Average</td>
                        {pillarAssessors.map(a => {
                          const aScores = pr.competency_results
                            .map(cr => cr.assessor_scores[a.id])
                            .filter(v => v !== undefined) as number[]
                          const avg = aScores.length > 0 ? aScores.reduce((s, v) => s + v, 0) / aScores.length : null
                          const ac  = avg !== null ? scoreColor(avg) : null
                          return (
                            <td key={a.id} className="py-2.5 px-3 text-center">
                              {avg !== null ? (
        ...
        return (
          <Page key={pr.pillar.id}>
            ...
            <div className="px-12 py-6 space-y-5">
              ...
              <div className="avoid-break">
                ...
                <div className="rounded-xl border border-slate-100 overflow-hidden">
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {pr.competency_results.map((cr, ci) => {
                        const rephraseRaw = aiCache[`evidence_rephrase_${cr.competency.id}`] || aiCache[`evidence_bullets_${cr.competency.id}`]
                        ...
                        return activeAssessors.map((a, ai) => {
                          ...
                          const displayText = typeof rephrased === "string"
                            ...
                            : rawEv
                          const sc = score !== undefined ? scoreColor(score) : null
              ...
              <ExpertPanel text={pillarAiStory} label={`Expert ${pr.pillar.name} Analysis`} color="blue" />
              {pillarAiVariance && <ExpertPanel text={pillarAiVariance} label="Intra-Pillar Variance Note" color="amber" />}
            </div>
      ...
      {hasDevPage && (
        <Page>
          ...
          <div className="px-12 py-6 space-y-5">
                    ...
                    .map(cr => {
                      ...
                      return (
                        <div key={cr.competency.id} className="rounded-xl border overflow-hidden" style={{ borderColor: cc.border }}>
                          <div className="flex items-center justify-between px-3 py-2.5" style={{ background: cc.bg }}>
                            <div>
                              <p className="text-[9px] font-bold uppercase tracking-wider" style={{ color: cc.text, opacity: 0.7 }}>{pr.pillar.name}</p>
                              <p className="text-xs font-semibold text-slate-700">{cr.competency.name}</p>
                            </div>
                            <span className="text-lg font-black tabular-nums" style={{ color: cc.text }}>{cr.weighted_avg.toFixed(2)}</span>
                          </div>
                          {insight && (
            ...
            <DevelopmentRoadmap raw={aiCache["development_plan"]} />
          </div>
        ...
        <div className="px-12 py-6 space-y-5">
          ...
          <div className="avoid-break">
            ...
            <div className="grid grid-cols-2 gap-4">
              <ExpertPanel text={aiCache["profile_interpretation"]} label="Profile Interpretation" color="slate" />
              <ExpertPanel text={aiCache["red_thread"]}             label="Hidden Pattern"          color="purple" />
            </div>
          </div>
          ...
          <div className="avoid-break">
            ...
            <div className="grid grid-cols-2 gap-4">
              <ExpertPanel text={aiCache["strengths_narrative"]}  label="Demonstrated Strengths" color="emerald" />
              <ExpertPanel text={aiCache["weaknesses_narrative"]} label="Key Development Gaps"    color="amber"   />
            </div>
          </div>
          ...
          <div className="avoid-break">
            ...
            <div className="grid grid-cols-2 gap-4">
              <ExpertPanel text={aiCache["verdict_explanation"]} label="What Drove This Verdict" color="blue"  />
              <ExpertPanel text={aiCache["what_would_change"]}   label="What Would Change It"    color="amber" />
            </div>
          </div>

          <div className="avoid-break">
            <SectionTitle>Phase 4 — Future Focus</SectionTitle>
            <ExpertPanel text={aiCache["forward_focus"]} label="What the Candidate Must Focus On" color="blue" />
          </div>
          ...
          <div className="avoid-break">
            ...
            <ExpertPanel text={aiCache["recommendation"]} label="Expert Recommendation" color="blue" />
          </div>
        </div>
