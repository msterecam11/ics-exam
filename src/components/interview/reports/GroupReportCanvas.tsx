    ...
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-0.5 px-4">
        ...
        <span className="font-black tracking-wider uppercase mt-1 text-center leading-tight text-[10px]"
          style={{ color: labelCol }}>{readyPct}% READY</span>
      </div>
    </div>
  )
}
...
export default function GroupReportCanvas({
  ...
  const bannerProps = { groupName: group.name, today, readiness: `${readyCount} Ready`, readinessBg, readinessBorder, readinessColor }
          ...
          <div className="px-12 py-6 space-y-5">
            ...
            <div className="avoid-break flex items-start justify-between gap-6 pb-5 border-b-2 border-slate-100">
              ...
              <div className="flex flex-col items-center gap-1.5 shrink-0">
                <div className="rounded-2xl px-8 py-4 text-center border-2" style={{ background: readinessBg, borderColor: readinessBorder }}>
                  <p className="text-4xl font-black tabular-nums" style={{ color: readinessColor }}>{avgScore.toFixed(2)}</p>
                  <p className="text-[10px] font-semibold mt-0.5" style={{ color: readinessColor }}>/ 5.00 AVG</p>
