...
const styles = StyleSheet.create({
  ...
  aiBox: { backgroundColor: "#eff6ff", borderRadius: 3, padding: 5, marginTop: 5 },
  aiLabel: { fontSize: 8, color: "#1d4ed8", fontFamily: "Helvetica-Bold", marginBottom: 2 },
  aiText: { fontSize: 8.5, color: "#1e40af", lineHeight: 1.4 },
    ...
    <Document title={`${candidate.full_name} — ${exam?.title}`}>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <Text style={styles.headerTitle}>ICS Aviation — Exam Report</Text>
        ...
        {answers.map((answer, idx) => {
          ...
          const scoreColor = sc >= q?.score ? "#059669" : sc > 0 ? "#d97706" : "#dc2626"
          return (
...
export function PDFDownloadButton({ candidateId, candidateName }: { candidateId: string; candidateName: string }) {
  ...
  return (
    <PDFDownloadLink
      ...
      fileName={`${candidateName.replace(/\s+/g, "-")}-report.pdf`}
