...
export async function parseProgramInput(body: any, partial: boolean): Promise<Result> {
  ...
  for (const k of ["certificate_enabled", "certificate_auto_release", "external_ics_certificate", "feedback_enabled", "feedback_mandatory", "feedback_anonymous", "progress_enforcement"] as const) {
    if (!has(k)) continue
